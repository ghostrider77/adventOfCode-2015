...
let adjust_lights (grid : bool array array) ({switch; topLeft; bottomRight} : instruction) : unit =
...
let calc_nr_lits_on (instructions : instruction list) : int =
  ...
  Array.fold_left (fun acc row -> Array.fold_left (fun count on -> if on then count + 1 else count) acc row) 0 grid
