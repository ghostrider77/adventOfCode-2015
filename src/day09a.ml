...
let find_shortest_path (distances : int Distances.t) : int =
  ...
  List.fold_left (fun acc p -> min acc (calc_path_distance p)) max_int perms


let () =
  let lines = In_channel.input_lines stdin in
  let distances = parse_input lines in
  let result = find_shortest_path distances in
