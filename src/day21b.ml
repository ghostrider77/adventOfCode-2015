...
let find_most_expensive_defeat (hit_points : int) (boss : player) : int =
  let players = generate_players hit_points in
  List.fold_left (fun acc (p, cost) -> if does_player_win p boss then acc else max cost acc) 0 players
...
let () =
  ...
  let result = find_most_expensive_defeat hit_points boss in
